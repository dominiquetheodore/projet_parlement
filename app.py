...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' in login_session and (login_session['email']=='dominique.theodore86@gmail.com' or login_session['email']=='jimmy.joplin@gmail.com'):
...
def editPQ(id):
	...
	if request.form['answer']:
            editedpq.answer = request.form['answer']
...
def deputiesdb():
    ...
    deputies = session.query(Deputy).order_by(Deputy.name).all()
    params = dict()
    #params['deputies'] = deputies
    #return render_template('deputies_test.html',**params)
